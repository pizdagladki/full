...
// TestRequireAuth verifies criterion: 5 — unauthenticated requests are
// rejected with 401 (no cookie, expired/missing session, or a resolve
// error), and a valid session sets the resolved user id in context.
func TestRequireAuth(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// criterion: 5 — no cookie returns 401 (unauthenticated)
		...
		{
			// criterion: 5 — expired/missing session returns 401
		...
		{
			// criterion: 5 — a valid session is authenticated and reaches the handler
