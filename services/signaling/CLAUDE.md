...
## Responsibility
- WebRTC-signaling control channel over WebSocket (`coder/websocket`).
- Will later relay SDP/ICE messages between the two peers of a room and perform server-side blink-time arbitration — those are OUT OF SCOPE downstream issues, not implemented here.
