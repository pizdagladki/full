...
- **Claimable** = (a) no assignee, OR (b) a STALE claim, OR (c) already assigned to YOU by a crashed prior cycle (resume/redo it). A claim is STALE when the latest `🤖 [CLAIM]` comment is older than 30 min AND nothing has happened SINCE that claim comment: no new commit/push to the branch, no review submitted after it, and no label change after it (for an issue — still no linked PR created after it). Judge progress RELATIVE TO the claim's timestamp, NOT the mere existence of an older review — an approval from a previous round must never keep a fresh claim alive forever. Read claim age via `mcp__github__get_pull_request_comments` (PRs) or `gh api graphql` (`…comments(last:20){nodes{author{login} body createdAt}}`). Recover a stale claim by claiming it exactly as above. This reclaims work orphaned by a crashed cycle (assignment is a lease, not a permanent lock).
...
2. MERGE-RECOVERY. An open PR that GitHub will never merge on its own because its branch is no longer mergeable — even though the fleet already finished reviewing it. This is what un-sticks an approved + auto-merge-armed PR once `main` moves under it: without it the first PR of a batch merges and every other approved PR hangs forever behind `main` (see fleet-flow.md §Гейт). Consider only PRs NOT labeled `needs-work` (a `needs-work` PR belongs to CHANGES → address.md, which already merges `main` and resolves conflicts itself). For each remaining open PR read `gh pr view <N> --json mergeStateStatus,mergeable,reviewDecision,autoMergeRequest,headRefName,isDraft` (Bash). It matches when ANY of:
   - `mergeable == CONFLICTING` (state `DIRTY`) — a real conflict with `main`, at ANY review state; OR
   - `reviewDecision == APPROVED` AND `mergeStateStatus == BEHIND` — approved but fallen behind `main`; OR
...
DEADLOCK CIRCUIT BREAKER: if open `task`+`owner-agreed` issues exist but none qualify (all with open blockers / a suspected A↔B cycle), and there's no changes/recovery/review work → apply `needs-human` to the oldest blocked issue (`mcp__github__update_issue`), print `[SELECT] deadlock -> needs-human #<N>` then `WORK_QUEUE_EMPTY`, exit.
