---
name: feeder
description: Decompose the product spec under docs/ into many small, dependency-ordered GitHub issues for the work-cycle fleet. Idempotent on re-runs; when the spec has CHANGED since an issue was written, emits reconcile tasks instead of silently skipping. Human-gated via labels. Run explicitly; never auto-trigger.
...
5. **Load the EXISTING backlog as FULL context — not just titles.** List ALL issues, open AND closed (`mcp__github__list_issues` / `mcp__github__search_issues`, repo `pizdagladki/full`), then read the COMPLETE body of EACH via `mcp__github__get_issue`: Goal, Acceptance criteria, Blocked by, Service/area, Out of scope, Context, and the hidden `<!-- fdr-* -->` fingerprint. **Also note each issue's STATE — open vs closed/merged** (a closed issue means its code is already shipped on `main`); you need it in Phase 1 to tell a not-yet-built task (re-triage) from a shipped one (needs a reconcile task) when the spec has moved under it. This is the authoritative record of what already exists. Read full bodies (NOT titles) because tasks overlap — even more so here: you need each existing task's exact scope, acceptance criteria, zone and Out-of-scope to (a) never duplicate or re-cover already-issued work, (b) never contradict or collide with an existing task's zone/scope/boundaries, (c) attach `Depends on #<real-number>` to the right existing issues, and (d) detect when the UPDATED spec now asks for DIFFERENT behavior than an existing issue already locked in. Carry this backlog into Phase 1 as the immovable baseline, and seed the Phase 2 `slug → issue#` map from these fingerprints.
...
**Build on the existing backlog (from Phase 0 step 5) — do not restart it.** Treat every already-existing issue as a fixed node in the graph. Plan ONLY genuinely new units beyond it. New units must respect existing tasks' zones and Out-of-scope so they don't overlap, and wire `Depends on` onto the real numbers of the existing issues they build on, exactly as for new-to-new dependencies.

When a unit you would emit OVERLAPS an existing issue (matching fingerprint, OR overlapping scope / acceptance criteria / zone — judged from its FULL body, NOT its title), do NOT blindly drop it: **the spec may have MOVED since that issue was written.** Compare the unit's updated-spec acceptance criteria against the existing issue's criteria:
- **Unchanged** — the spec still wants exactly what that issue already covers → drop the unit, reuse its number (the normal idempotent case).
- **Diverged** — the updated spec now requires different or additional behavior → do NOT silently drop. What you do depends on the existing issue's STATE (from Phase 0 step 5):
  - **Closed / merged (code already shipped on `main`)** → emit a NEW **reconcile** task: Goal = bring `<zone>` in line with the updated spec; Acceptance criteria = ONLY the changed/added behaviors, stated verifiably, and explicitly name which existing behavior must be **PRESERVED**; Service/area = the same zone; Out of scope = the parts that did NOT change; Context = `Modifies existing code shipped by #<orig> (see its merged PR); source: <updated spec doc>`. This is a forward modification ON TOP of the existing code — NOT a from-scratch reimplementation and NOT a bug report. Fingerprint `fdr-<area>-<slug>-reconcile` (distinct from the original, so re-runs stay idempotent). `Depends on #<orig>` if still relevant.
  - **Open / not yet implemented** → do NOT duplicate and do NOT silently drop — the existing (possibly `owner-agreed`) issue's criteria are simply STALE. FLAG it for human re-triage in Phase 3 (`#<n>: criteria diverged from updated spec`); do NOT edit an approved task yourself — the human updates or re-approves it.
  - If a `…-reconcile` issue ALREADY exists for this delta (by fingerprint) → treat it as any existing issue: skip if its criteria still match the spec; if the spec diverged AGAIN beyond it, FLAG for human in Phase 3 — NEVER auto-stack reconcile-on-reconcile.

When a run is capped to N issues, N counts NEW issues created (new units AND reconcile tasks) — skipped/existing ones do not count.
...
2. Check existence ACROSS ALL ISSUES regardless of state or label (an approved issue has lost `proposed`; a rejected one is closed — do NOT re-create either):
   `mcp__github__search_issues` with query `repo:pizdagladki/full type:issue fdr-<area>-<short-slug> in:body` (no state filter → searches open AND closed); for a reconcile unit also search `fdr-<area>-<short-slug>-reconcile`.
   - If found (by fingerprint) OR already covered by an existing issue from the Phase 0 backlog (by scope/meaning, from its full body) → apply the Phase 1 criteria-diff rule, NOT a blind skip:
     - criteria UNCHANGED → record its number in the map, SKIP creation;
     - criteria DIVERGED + existing issue closed/merged → create the **reconcile** task (fingerprint `…-reconcile`) per Phase 1;
     - criteria DIVERGED + existing issue open → SKIP creation, add it to the Phase 3 human-re-triage list.
   - If not found and genuinely new → create it.
...
Print to stdout: counts (created — split into **new** vs **reconcile** — vs skipped-existing), the dependency graph (slug → #number → its deps), **the `[RE-TRIAGE]` list — open existing issues whose criteria diverged from the updated spec (`#<n>: <one-line delta>`), which the human must update/re-approve**, and a list of every `Manual prerequisite (human)` you emitted, grouped, so the human knows what to set up before approving the affected issues. Remind that nothing is in the fleet's queue until a human adds `owner-agreed`; that reconcile tasks modify already-shipped code and should be approved with that in mind; and that dependencies must be approved in order (a task whose blocker is unapproved will wait; `work-cycle`'s deadlock breaker eventually flags it `needs-human`).
