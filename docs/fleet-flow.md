...
5. Цикл реализации: implement → PR → review → (при правках address ↔ review) → approve + auto-merge → (если ветка отстала от main или конфликтует до мержа) unblock → re-review → мерж.
...
2. UNBLOCK — открытый PR БЕЗ `needs-work` (PR с `needs-work` идёт в CHANGES → address, тот сам
   ...
   unblock. Можно брать и свой PR — это механический ре-синк, не ревью (само-аппрува не происходит).
...
3. REVIEW — открытый PR без `needs-work` (те в CHANGES), где ты не автор и не последний пушивший,
   ...
   оказался бы вне всех категорий и осиротел. `APPROVED` исключён (едет в мерж либо в UNBLOCK).
...
5. Вердикт:
    - GOOD = нет блокеров, нет apply-находок Copilot, все ожидаемые CI-джобы = success. Сначала
      ...
      `good-but-unmergeable -> unblock`, выход — ветку приведёт в порядок UNBLOCK, а
      ...
      до мержа PR можно было переподобрать в UNBLOCK — не оставлять одобренный PR за вышедшим
      воркером).
...
**UNBLOCK** (без `needs-work`; `gh pr view --json mergeStateStatus,mergeable,reviewDecision,autoMergeRequest`;
...
defer to unblock, выход) → `create_pull_request_review` APPROVE + `gh pr merge --auto --squash`
...
`unblock.md` (без ревью): `gh pr view --json mergeStateStatus,mergeable,reviewDecision,autoMergeRequest`
...
встаёт. Закрывает это шаг **unblock.md** (категория UNBLOCK в select.md): он `gh pr
...
— это зона человека, флот туда не пишет. Держим как апгрейд; пока хватает unblock.md.
...
- Зависший мерж: одобренный PR, отставший от main или конфликтующий, ловит категория UNBLOCK
  и чинит unblock.md (`gh pr update-branch` / merge+coder) — без неё одобренные PR висели бы вечно за
...
- Синк только merge, не rebase: force-push запрещён (`settings.json`) и заблокирован — rebase+push был
  бы тупиком; merge держит push fast-forward (implement/address/unblock).
- Армирование auto-merge проверяется: review.md/unblock.md убеждаются, что auto-merge реально включён
