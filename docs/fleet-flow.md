...
- `skills/work-cycle/` — оркестратор: `SKILL.md` + `steps/{select,implement,review,address,recover}.md`.
...
- локальный код (git worktree/merge/push, make, gofmt, typos) — Bash.
...
5. Цикл реализации: implement → PR → review → (при правках address ↔ review) → approve + auto-merge → (если ветка отстала от main или конфликтует до мержа) merge-recovery → re-review → мерж.
...
один из `steps/{implement,review,address,recover}.md` по типу, печатает маркеры и завершается.
...
2. MERGE-RECOVERY — открытый PR БЕЗ `needs-work` (PR с `needs-work` идёт в CHANGES → address, тот сам
   мержит main и решает конфликты), который GitHub сам не вмержит, потому что ветка перестала быть
   мёржабельной (по `gh pr view --json mergeStateStatus,mergeable,reviewDecision,autoMergeRequest`):
   `mergeable=CONFLICTING` (конфликт с main, при любом статусе ревью); ИЛИ approved + `BEHIND`
   (одобрен, но отстал от main); ИЛИ approved + мёржабелен, но auto-merge так и не армнулся. Тип
   recover. Можно брать и свой PR — это механический ре-синк, не ревью (само-аппрува не происходит).
   Это и есть страховка от зависания одобренных PR (без неё первый PR пачки мержится, остальные висят
   за main вечно — см. §Гейт).
3. REVIEW — открытый PR без `needs-work` (те в CHANGES), где ты не автор и не последний пушивший,
   которому ещё нужно решение ревью (`reviewDecision` ≠ `APPROVED` — т.е. `REVIEW_REQUIRED`,
   `CHANGES_REQUESTED` или не задан). `CHANGES_REQUESTED` обязателен к включению: после address
   (снял `needs-work`, но прежний REQUEST_CHANGES висит, если репо не сбрасывает старые ревью) PR
   оказался бы вне всех категорий и осиротел. `APPROVED` исключён (едет в мерж либо в MERGE-RECOVERY).
   claimable. Иначе тип review.
4. NEW ISSUE — open issue с обоими лейблами `task` и `owner-agreed`, claimable, все блокеры
...
осиротевшую крашем работу. Прогресс меряется ОТНОСИТЕЛЬНО штампа claim (новый push / review / смена
лейбла ПОСЛЕ него), а не по факту существования старого ревью — иначе approve прошлого раунда держал
бы claim вечно и одобренный-но-застрявший PR никто бы не переподобрал.
...
5. Вердикт:
    - GOOD = нет блокеров, нет apply-находок Copilot, все ожидаемые CI-джобы = success. Сначала
      ГЕЙТ МЁРЖАБЕЛЬНОСТИ: `gh pr view <N> --json mergeStateStatus,mergeable` — если `BEHIND` или
      `CONFLICTING`, НЕ аппрувить в этот цикл (аппрув на отставшей/конфликтной ветке всё равно
      слетит при обязательном апдейте ветки и сгорит): снять себя из assignee, печать
      `good-but-unmergeable -> merge-recovery`, выход — ветку приведёт в порядок MERGE-RECOVERY, а
      свежий цикл ревью одобрит уже актуальный head. Иначе: Approve через MCP +
      `gh pr merge <N> --auto --squash`, ПРОВЕРИТЬ что auto-merge реально армнулся (exit 0 +
      `autoMergeRequest` ≠ null; иначе `CYCLE_ERROR`), снять себя из assignee (чтобы при отставании
      до мержа PR можно было переподобрать в MERGE-RECOVERY — не оставлять одобренный PR за вышедшим
      воркером).
...
### recover.md — вернуть застрявший PR в мёржабельное состояние

Механический ре-синк одобренного/конфликтного PR, который GitHub сам не вмержит. НЕ ревью — аппрува
здесь нет (даже на своём PR). Ты уже заклеймил PR.

1. Свежее состояние: `gh pr view <N> --json mergeStateStatus,mergeable,reviewDecision,autoMergeRequest,...`.
...
2. `BEHIND` без конфликта: `gh pr update-branch <N>` (мерж main в ветку, без force-push; `--rebase`
   НЕ использовать). Это новый push от тебя → при «require approval of most recent push» аппрув
   слетит и `reviewDecision` станет `REVIEW_REQUIRED` — другой аккаунт переодобрит актуальный head
   через REVIEW (если аппрув не слетает — auto-merge сработает сразу по зелёному). Auto-merge
   переживает апдейт.
3. `CONFLICTING`/`DIRTY`: worktree на head ветки, `git merge origin/main`, конфликт решает `coder`
   ...
   коммент, выход. Новый push так же отправит PR на re-review.
4. Догарантировать armed auto-merge (если слетел — `gh pr merge <N> --auto --squash`, проверить).
   Раунды/лейблы/борд не трогать.
5. Снять claim. Если пушил (п.2/3) — ты теперь последний пушивший и сам ревьюить/аппрувить этот PR
   не можешь (это сделает другой аккаунт через REVIEW). Выход.
...
`steps/{implement|review|address|recover}.md` → `[CYCLE] done` / `[CYCLE] end #N` → процесс умирает.
...
**MERGE-RECOVERY** (без `needs-work`; `gh pr view --json mergeStateStatus,mergeable,reviewDecision,autoMergeRequest`;
match: `CONFLICTING` ИЛИ approved+`BEHIND` ИЛИ approved+мёржабелен+auto-merge не армнут; claim; можно
свой PR) · **REVIEW** (без `needs-work`, `reviewDecision`≠`APPROVED` — вкл. `CHANGES_REQUESTED` после address; claim: `update_issue` assignees=[you]
→ пауза 1–3 с → `get_pull_request` перечитать → проигрыш = yield) · **NEW ISSUE** (`list_issues`
`task,owner-agreed`; блокеры `Depends on #X` через `get_issue`; claim тот же) → `[PICKED] type=… #N`
либо `WORK_QUEUE_EMPTY`.
...
тредов `add_issue_comment` + `gh api graphql resolveReviewThread` → вердикт: **GOOD** — гейт
мёржабельности `gh pr view --json mergeStateStatus,mergeable` (BEHIND/CONFLICTING → снять assignee +
defer to merge-recovery, выход) → `create_pull_request_review` APPROVE + `gh pr merge --auto --squash`
(проверить, что армнулся) + снять себя из assignees; **BAD** — REQUEST_CHANGES + `update_issue` (бамп
`round-*`, `needs-work`/`needs-human`) + снять себя из assignees.

`recover.md` (без ревью): `gh pr view --json mergeStateStatus,mergeable,reviewDecision,autoMergeRequest`
→ `BEHIND` → `gh pr update-branch` (merge main, без force) · `CONFLICTING` → `git worktree add` +
`git merge origin/main` → **coder** резолвит → `git push` (fast-forward); не вышло → `needs-human` →
догарантировать armed auto-merge → снять `update_issue` assignees. Новый push → PR уходит на re-review.
...
`work-cycle` → `select` → {`implement` | `review` | `address` | `recover`}; `implement`/`address`/`recover` → **coder**;
...
Как ветка держится «не отстающей» (без этого одобренные PR висят). GitHub auto-merge сам отставшую
ветку под main НЕ подтягивает, а merge queue не настроен (в `ci.yml` нет триггера `merge_group`).
Поэтому как только первый PR пачки вмержился, остальные одобренные оказываются `BEHIND` и auto-merge
встаёт. Закрывает это шаг **recover.md** (категория MERGE-RECOVERY в select.md): он `gh pr
update-branch` (merge main в ветку, force-push не нужен) на отставших и резолвит настоящие конфликты
через `coder`. Синк всегда merge, не rebase: rebase переписал бы историю и потребовал force-push,
который запрещён (`settings.json`) и заблокирован branch protection. Апдейт ветки — это новый push,
он сбрасывает аппрув (require approval of most recent push) → PR переодобряет другой аккаунт через
REVIEW; так система сходится. Размен: при высоком темпе мержей возможен churn (апдейт → переревью →
снова отставание). Полностью убирает его только GitHub Merge Queue (нативно ребейзит и
перепроверяет очередь), но он требует правок в `.github/` (триггер `merge_group` в `ci.yml` + ruleset)
— это зона человека, флот туда не пишет. Держим как апгрейд; пока хватает recover.md.
...
- Фикс дедлока ре-ревью: review.md и в BAD, и в GOOD снимает себя из assignee — PR возвращается в пул
  (GOOD-путь больше не оставляет одобренный PR за вышедшим воркером).
- Зависший мерж: одобренный PR, отставший от main или конфликтующий, ловит категория MERGE-RECOVERY
  и чинит recover.md (`gh pr update-branch` / merge+coder) — без неё одобренные PR висели бы вечно за
  main (первый из пачки мержится, остальные нет). Claim-staleness меряет прогресс относительно штампа,
  а не по факту старого аппрува, иначе застрявший PR был бы вечно non-claimable.
- Синк только merge, не rebase: force-push запрещён (`settings.json`) и заблокирован — rebase+push был
  бы тупиком; merge держит push fast-forward (implement/address/recover).
- Армирование auto-merge проверяется: review.md/recover.md убеждаются, что auto-merge реально включён
  (`autoMergeRequest` ≠ null), иначе `CYCLE_ERROR` — не рапортовать успех на PR, который не вмержится.
...
`[REVIEW]`, `[COPILOT]`, `[ADDRESS]`, `[RECOVER]`. Реальное время даёт хук, смысл — маркеры;
...
Получение логов (текст; `tail`/`grep`, одинаково на всех трёх):
    ...
    grep -nE '\[(CYCLE|SELECT|PICKED|IMPLEMENT|PR|REVIEW|COPILOT|ADDRESS|RECOVER)\]' ~/fleet-logs/fleet.log   # только вехи
