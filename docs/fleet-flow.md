...
- `agents/adjudicator.md` (Opus, `tools: Read`) — выносит ФИНАЛЬНЫЙ вердикт (apply/dismiss + GOOD/BAD)
  по находкам панели из СВЕЖЕГО окна. Нужен в implement-цикле: там родитель писал код и предвзят, поэтому
  вердикт делегируется свежему сабагенту (контекстная декорреляция без нового `claude -p`). В review.md
  родитель сам свежий процесс — там adjudicator не нужен.
...
GitHub и судит дифф как чужой — поэтому ЗДЕСЬ вердикт выносит сам родитель (он уже свежий; отдельный
`adjudicator`-сабагент не нужен, в отличие от контаминированного implement-цикла). Несущая
объективность — в CI и маппинге критерий→тест; ИИ-панель совещательна (см. §Гейт).
...
(только sensitive-путь) → адъюдикация (UNTESTED = механический блокер; dismiss → дословно в коммент)
+ резолв тредов → вердикт: **GOOD** — гейт мёржабельности (`BEHIND`/`CONFLICTING` → снять assignee +
...
`review` → те же панель-агенты, но вердикт выносит сам (свежий) родитель — без **adjudicator**;
...
от свежести родителя: в **implement** (первое ревью, родитель писал код → предвзят) вердикт делегируется
свежему сабагенту **`adjudicator`** — контекстная декорреляция без нового процесса; в **review.md**
(ре-ревью / legacy / поздний Copilot) родитель сам новый `claude -p`, поэтому судит напрямую. В обоих
...
sensitive-пути) ревьюит дифф; `adjudicator` выносит вердикт (в implement). Это даёт свежесть (ревьюер
...
- Объективность self-review = свежее окно у того, кто выносит вердикт. В implement (первое ревью)
  вердикт делегируется свежему `adjudicator`-сабагенту, а не контаминированному родителю-оркестратору.
  В review.md (ре-ревью / legacy / поздний Copilot) родитель сам новый `claude -p`. Несущие гейты
...
`--model opus` + ultracode — самый дорогой режим (дешевле — `--effort xhigh` вместо `--settings`).
